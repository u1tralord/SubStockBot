...
def place_sell(sub_reddit, quantity, price, user_id):
    ...
    sub_reddit_id = db.get_single('SUB_REDDITS', ['id'], ('name', '=', sub_reddit))[0]
    quantity_owned = 0
    owned_stocks = db.get('OWNED_STOCKS', ['quantity', 'id'],
                          (('user_id', '=', user_id), ('sub_reddit_id', '=', sub_reddit_id)))
    ...
    if quantity_owned >= quantity:
        ...
        while quantity_remaining > 0:
            ...
            if row_quantity >= quantity_remaining:
                db.update('OWNED_STOCKS', {"quantity": str(row_quantity - quantity_remaining)},
                          ('id', '=', str(owned_stocks[index][1])))
            ...
            if sell[0] > 0:
                for buy in possible:
                    ...
                    if buy[0] > sell[0]:
                        ...
                        quantity = buy[0]
                    db.query('UPDATE MARKET SET quantity = quantity-%s WHERE id=%s OR id=%s',
                             (quantity, buy[3], sell[3]))
